...
class VariancePreserve(Base):
    ...
    def make_noised_reals(self, x_0, noise, t):
        # noised_reals = self.mu_t(t, x_0) + self.sigma_t(t, x_0) * noise
        t = t.view(t.size(0), 1)
        noised_reals = self.alpha_t(1. - t) * x_0 + self.sigma_t(t) * noise
        return noised_reals

    def make_targets(self, x_0, noise, t):
        t = t.view(t.size(0), 1)  # TODO: make this more adaptable
        noised_reals = self.make_noised_reals(x_0, noise, t)
        num = torch.exp(-self.T(1. - t)) * noised_reals - torch.exp(-0.5 * self.T(1. - t)) * x_0
        denum = 1. - torch.exp(- self.T(1. - t))
        targets = - 0.5 * self.beta(1. - t) * (num / denum)
        return targets

    # def u_t(self, t, x, x_1):
